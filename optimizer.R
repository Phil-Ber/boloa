...
# # )
# lf <- list(c(261, 336, 345, 470, 479, 567, 600, 672),
#            c(261,392,461,460,478,563,600,617,629,637,653,672,710),
#            c(262,284,392,458,468,478,493,524,549,565,575,599,622,629,638,644,653,665,672,710)
# )
# ## Upper peak limit
...
# # )
# uf <- list(c(268, 340, 347, 474, 482, 571, 603, 676),
#            c(269,394,463,473,482,569,603,620,632,641,655,676,713),
#            c(268,289,394,460,473,482,496,526,553,569,579,604,624,631,640,646,655,667,675,712)
# )
...
lf <- list()
uf <- list()
# A method of peak detection in order to retrieve peak ranges, not as accurate as CentWave or other methods.
for (sampleN in 1:length(msfile)) {
  raw_data <- readMSData(files = msfile[sampleN], pdata = new("NAnnotatedDataFrame"), mode = "onDisk")
  ...
  for (i in 2:length(newvals)) {
    if (is.na(newvals[i]) != TRUE & is.na(newvals[i - 1])) {
      minranges <- c(minranges, rtime(raw_data)[i])
    }
    if (is.na(newvals[i]) == TRUE & is.na(newvals[i - 1]) == FALSE) {
      maxranges <- c(maxranges, rtime(raw_data)[i])
    }
  }
  lf[[sampleN]] <- minranges
  uf[[sampleN]] <- maxranges
}
  ...
  tryCatch(
    {
      ...
      for (file_index in 1:length(msfile)) {
        ...
        fres <- c(fres, pmax(0, 1 - abs(abs(n_corr) - length(lf[[file_index]])) / ((abs(n_corr) - length(lf[[file_index]]))^2)/2))
...
#   edf <- edf[edf[,1] != 1,]
# }
# write.csv(edf, "/exports/nas/berends.p/boloa/randomgen_param_for_optimization.csv", row.names=FALSE)
edf <- read.csv("/exports/nas/berends.p/boloa/randomgen_param_for_optimization.csv", header
                =TRUE, stringsAsFactors=FALSE)
edf <- na.omit(edf)
tries <- 0
prev_add <- tail(edf, n = 1)
...
while (mean(edf[,1]) >= 0.10 | tries < 100) {
  ...
  #   for (i in 1:length(end_vals)) {
  #     GSfactor <- -cor(edf[,i+4], edf[,1])
  #     npars <- c(npars, end_vals[i] * ((GSfactor^-(1/2)) + 1))
  #   }
  #   edf <- edf[edf[,1] != 1,]
  # }
  else {
  ...
  prev_add <- new_add
  write.table(new_add, file = "/exports/nas/berends.p/boloa/optimize_screen_10.csv", sep = ",", append = TRUE, quote = FALSE,
