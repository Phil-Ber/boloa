...
for (i in 1:2) {
    ...
    randvals <- c(
        ...
        runif(1, -1.1, 1), #nds.p/boloa/optimize_screen_2.csv", row.names=FALSE)

tries <- 0
prev_add <- data.frame(c(2,2,3))
while (mean(edf[,1]) >= 0.10 |
    edf <- rbind(edf, detection(1, randvals))
    edf <- edf[edf[,1] != 1,]
}
all_vals <- c()
write.csv(edf, "/exports/nas/bere tries < 1000) {
    edf <- tail(edf, n = 1000)
